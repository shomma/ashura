...
`.env` がないローカル環境では、アプリと `npm run sqlite:init` は既定で
`prisma/data/ashura.db` を使います。別のSQLiteファイルを使う場合は
`DATABASE_URL=file:./data/ashura.db` のように `prisma/schema.prisma` からの
相対パス、または `file:/absolute/path/to/ashura.db` を指定してください。
